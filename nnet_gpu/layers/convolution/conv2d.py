...
class conv2d(Layer):
    def __init__(self, num_kernels=0, input_shape=None, kernel_size=0, kernels=None, activation=echo, biases=0,
        ...
        if self.kernels is None:
            ...
            self.kernels, self.biases = self.init_kernel_bias(self.channels, self.kernel_size, num_kernels, std=std,
                                                              dtype=self.dtype)
        else:
        ...
        if padding == None:
            self.padding = self.cal_padding(self.row, self.kernel_size[0], self.stride[0],
                                            self.dilation[0]), self.cal_padding(self.col, self.kernel_size[1],
                                                                                self.stride[1], self.dilation[1])
        if out_row is None:
            self.out_row = self.cal_outsize(self.row, self.kernel_size[0], self.stride[0], self.padding[0],
                                            self.dilation[0])
        else:
            self.out_row = out_row
        if out_col is None:
            self.out_col = self.cal_outsize(self.row, self.kernel_size[1], self.stride[1], self.padding[1],
                                            self.dilation[1])
        else:
    ...
    def init_back(self):
        ...
        self.d_ker = conv2d(input_shape=(self.row, self.col, self.batches), kernels=grads, activation=echo,
                            stride=(1, 1), dilation=self.stride, padding=self.padding, backp=False,
                            out_row=self.kernel_size[0], out_col=self.kernel_size[1])
        self.d_inp = conv2dtranspose(input_shape=(self.out_row, self.out_col, self.num_kernels), kernels=self.kernels,
                                     activation=echo, stride=self.stride, padding=self.padding, dilation=self.dilation,
                                     backp=False, out_row=self.row, out_col=self.col)

    def init_kernel_bias(self, num_inp_channels, kernel_size, num_kernels, mean=0, std=0.01, dtype=cp.float32):
        weights = std * cp.random.randn(num_inp_channels, kernel_size[0], kernel_size[1], num_kernels,
                                        dtype=cp.float32) + mean
    ...
    def backprop(self, grads, layer=1):  # strides[batch,row,col,depth]
        ...
        if layer:
            ...
            assert d_inputs.shape == (self.batches, self.row, self.col,
                                      self.channels), f"{(self.batches, self.row, self.col, self.channels)},{d_inputs.shape}"
        else:
            d_inputs = 0
        if self.bias_is_not_0:
            with self.backp_stream:
