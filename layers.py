...
class conv2d:
	def __init__(self,num_kernels=0,input_shape=None,kernel_size=0,kernels=None,biases=0,stride=[1,1],padding=0,backp=True,name=None):		#padding=(ksz-1)/2 for same shape in stride 1
		...
		self.padded=np.zeros((self.batches,self.channels,self.row,self.col))
		self.param=(self.kernel_size*self.kernel_size*self.channels+1)*self.num_kernels
		# Take all windows into a matrix
		window=(np.arange(self.kernel_size)[:,None]*self.row+np.arange(self.kernel_size)).ravel()+np.arange(self.channels)[:,None]*self.row*self.col
		...
		# bind= np.arange(self.batches)[:,None]*self.channels*self.row*self.col+self.ind.ravel()		#for self.batches
	...
	def forward(self,inp):
		self.inp=inp.transpose(0,3,1,2)  #inp[batches,self.channels,self.row,self.col]
		...
		for i,img in enumerate(self.padded):		#img[self.channels,self.row,self.col]
			# windows(self.out_row*self.out_col, kernel_size*kernel_size*self.channels) . kernels(self.channels*kernel_size*kernel_size,self.num_kernels)
...
class dense:
	def __init__(self,num_out,input_shape=None,mean=0,std=0.1,name=None):
		self.type=self.__class__.__name__
		if name is None:
			self.name=self.__class__.__name__
		else:
